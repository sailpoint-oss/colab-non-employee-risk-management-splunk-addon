...
The SailPoint NERM(Non-Employee Risk Management) API is a RESTful API designed to manage and automate identity governance processes for non-employee identities within SailPoint platform. It allows you to handle non-employee onboarding, updates, risk assessments and offboarding efficiently.
...
1. Splunk Enterprise and Splunk Cloud instance with admin access
2. API Access
       Tenant Name
       API Token
            //Details on steps to be added here
...
3. Submit the Add-on for App vetting
   ...
   -- To submit, open a support ticket with Splunk support and provide the following
       The add-on package(.spl or .tgz file)
...
4. Upload the .spl file from the release package
5. Click Upload amd restart Splunk if prompted
...
3. Restart the Splunk instance
    splunk restart
Steps to Install Add-on on Splunk Cloud:

Method 3: Install on Splunk Cloud
1. Navigate to Splunk Cloud Admin Console > App > Browse More Apps
2. Search for the add-on and click install
3. Submit a request to Splunk support to get app cloud certified for installation
...
2. Select the data input
   // Add screenshot
3. Click Add New and fill in the required details.
   // Add screenshot
...
2. Search with Extracted Fields:
 index=<your_index> sourcetype=<sourcetype_name>

Troubleshooting

1. Data not ingesting Properly
...
2. Field Extractions Not working
...
2.Import the .tgz file in Splunk Add-on Builder.
  // Add the screenshot
